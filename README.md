# Compliance Test Suites for Arm System Architecture Standards

## Table of Contents

- [Introduction to Arm sysarch-acs](#introduction-to-arm-sysarch-acs)
- [Architecture Compliance Suite](#architecture-compliance-suite)
...
- [DRTM, System MPAM and mem_test](#drtm-system-mpam-and-mem_test)
...
<details>
<summary><h2>Architecture Compliance Suite</h2></summary>

**Architecture Compliance Suite** (ACS) is a collection of self-checking, portable C-based tests.
...
A few tests are executed by running the BSA and SBSA ACS Linux application which in turn depends on the BSA and SBSA ACS Linux kernel module.
...
<details>
<summary><h2>BSA Architecture Compliance Suite</h2></summary>
...
### Prebuilt images
Prebuilt images for each release are available in the prebuilt_images folder of the main branch. You can choose to use these images or build your own image by following the steps below. If you choose to use the prebuilt image, see the execution section below for details on how to run the application.
...
#### 1.1 Target Platform
##### To start the ACS build for platform using ACPI table or Device tree, perform the following steps:
...
3.  source edksetup.sh
4.  make -C BaseTools/Source/C
5.  For ACPI: source ShellPkg/Application/sysarch-acs/tools/scripts/acsbuild.sh bsa
6.  For DT: source ShellPkg/Application/sysarch-acs/tools/scripts/acsbuild.sh bsa_dt
...
Note: 'Shell.efi' is available in the [prebuilt_images/IR](prebuilt_images)
...
1. Create an image file which contains the 'Bsa.efi' file. For example:
  ...
  - sudo cp  "<path to application>/Bsa.efi" /mnt/acs/
  - sudo umount /mnt/acs
...
7. Copy the UART console output to a log file for analysis and certification.
  ...
  - For information on the BSA uefi shell application parameters, see the [User Guide](docs/bsa/arm_bsa_architecture_compliance_user_guide.pdf).
...
3. Boot the system to UEFI shell.
4. Run the executable 'Bsa.efi' to start the compliance tests. For details about the parameters,see the [User Guide](docs/bsa/arm_bsa_architecture_compliance_user_guide.pdf).
...
2. git clone https://github.com/ARM-software/sysarch-acs.git
...
6. ./bsa_setup.sh <local_dir/sysarch-acs>
7. ./linux_bsa_acs.sh
...
1. cd <sysarch-acs path>/linux_app/bsa-acs-app
...
```
  ...
  - For information on the BSA Linux application parameters, see the [BSA User Guide](docs/bsa/arm_bsa_architecture_compliance_user_guide.pdf)
...
<details>
<summary><h2>SBSA Architecture Compliance Suite</h2></summary>
...
The following steps describe how to build the SBSA kernel module and application using the build.sh script.
The build.sh script supports both native builds and cross-compilation.
- For Native Builds, run the script directly on the target machine.
- For Cross-Compilation, pass the Linux version and GCC tool version as script arguments.
...
The following steps describe how to build the SBSA kernel module and application for the system manually.
...
1. cd <sysarch-acs path>/linux_app/sbsa-acs-app
2. export CROSS_COMPILE=<ARM64 toolchain path>/bin/aarch64-linux-gnu-
3. make

Successful completion of above steps will generate executable file **sbsa**  in <sysarch-acs path>/linux_app/sbsa-acs-app
...
|-----------------------------|------------------------------------------------------------------------------|------------------------|
|pal_pcie_dev_p2p_support     |Return 0 if the test system PCIe supports peer to peer transaction, else 1    |817, 818, 881 and 882   |
|pal_pcie_is_cache_present    |Return 1 if the test system supports PCIe address translation cache, else 0   |880                     |
|pal_pcie_get_legacy_irq_map  |Return 0 if system legacy irq map is filled, else 1                           |878                     |
 ...
 - SBSA Test 888 (Check ECAM Memory accessibility) execution time depends on the system PCIe hierarchy. For systems with multiple ECAMs the time taken to complete can be long which is normal. Please wait until the test completes.
...
<details>
<summary><h2>DRTM, System MPAM and mem_test</h2></summary>
...
- To evaluate the correctness and consistency of a system's memory model, memory model consistency tests (litmus tests) can be optionally built into BSA UEFI application, the build and run steps are provided in [MEMTEST_README](mem_test/README.md).
...
* For feedback, please use the [GitHub Issue Tracker](../../issues).
* For support, contact **[support-systemready-acs@arm.com](mailto:support-systemready-acs@arm.com)**.
* Arm licensees may contact Arm through their partner managers.
